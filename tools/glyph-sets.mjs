...
//
// It covers the 'free' class only, and that is not merely a licence
// consequence: every free set is `phasesY: '0'`, and only those reproduce.
// The 26 legacy '0,0.5' sets provably do not — see PROVENANCE in
// glyph-registry.mjs for what that costs (measured: no transcript, 1 px of
// reported baseline). --plan flags them inline.
// ---------------------------------------------------------------------------
...
// A set generated with y-phases '0,0.5' cannot be regenerated byte-identically
// — its ½-phase rasters came from a pipeline that rounded pen y to an integer,
// and fontgen places a true 1/64 pen. Measured cost: none to any transcript,
// 1 px to the reported baseline of ½-phase lines (glyph-registry.mjs
// PROVENANCE has the numbers). The commands below are still the right ones to
// run; you just do not get the committed bytes back.
const legacyY = name => (PROVENANCE[name].phasesY ?? '').includes('0.5');
...
if (argv[0] === '--plan') {
  ...
  for (const cls of ['free', 'system', 'build']) {
    ...
    for (const n of ns) console.log(command(n) + (legacyY(n) ? '   # y-phases 0,0.5: will NOT match the original bytes' : ''));
  }
