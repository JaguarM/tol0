...
export const FAMILIES = [
  ...
  { name: 'page-downscale-816x1073', renderable: false,
    ...
    record: '16 documents in lab/base64/unidentified/ plus EFTA02154109 (83 pp), which is staged apart in lab/base64/courir-strech/ — that is the 17th this entry has always counted, not a new find. (1) and (4) measured on EFTA02715081 (58 pp, 475–477 two-column stems at every height floor 3..8). (2) says 40 pages, so it is one of the OTHER 15 — and the 666-stem figure once quoted for (1) is likewise not this document. Which one went unrecorded; the entry conflates at least two. ' +
      ...
      'TWO CAVEATS, so nobody inherits a wrong number: (1) the X arm of that scan is INVALID — every x kernel wider than 3.125 returned zero ink bands from the harness while the same kernel math works on an isolated test, so this experiment says NOTHING about the x kernel and the earlier per-glyph fit\'s "x tri(4)" was never confirmed here. (2) The per-axis edge ramps are COUPLED through the core threshold: widening y alone raised the measured x ramp from 2.09 to 2.69, because softer cores move where the ≤96 "core" test fires. The claim "x is blurred more than y", which rests on ramps, is therefore weaker than stated above and needs an axis-independent measurement before it is relied on.' },
