...
export const FAMILIES = [
  ...
  { name: 'page-downscale-816x1073', renderable: false,
    ...
    action: 'NOT a face hunt, and a wider roster will not close it — the pixels are a 300 dpi GRAYSCALE source area-averaged 8/25 down to 96 dpi ("bitonal" here until 2026-07-29, refuted by the mid-tone census and by the grayscale control that reproduces the fingerprint). Tolerance 0 is reachable only by reproducing THAT: rasterize antialiased at 3.125× and apply the same box resample. Until someone builds that, these documents have no pool and no □-free read. Do NOT widen a roster or loosen a tolerance against them. ' +
      ...
      'Best over cour/consola/lucon/NimbusMono × em64 2540..2570 × pen at 1/4 source px × {area, area±½, triangle, box 3.125..7, gauss, two-step} × {trunc, half-up, ceil}: cour.ttf em64 2568, box(3.5), truncate — Σ|Δ| 295 over 110 bytes, 36 bytes differ, worst 25. NOT byte-exact, so the recipe is still wrong somewhere. ' +
      'What the residual says: the glyph CORE agrees within a few levels while the target carries a soft halo one pixel further out than any centred kernel reproduces (Δ +25 +21 +15 +14 +12 on the outer ring), and the fit keeps asking for more blur and more em. So the missing piece is upstream of the resample — a bolder/blurrier source render than ftclone (stem darkening?), or byte ROUNDING at an intermediate step (300→150→96), which a continuous kernel model cannot express. Try those before widening the roster again. ' +
